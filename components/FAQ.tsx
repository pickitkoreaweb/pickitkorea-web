...
const FAQ: React.FC<FAQProps> = ({ setPage }) => {
  ...
  const faqs = [
    ...
    {
      question: "메탈 명함으로 스마트폰 연동이 가능한가요? (QR)",
      answer: "네, PICKIT 메탈 명함은 뒷면에 정밀 레이저 각인된 QR코드를 통해 스마트폰과 연동됩니다. \n\n상대방이 카메라로 QR코드를 스캔하면, 저장해둔 연락처, 포트폴리오, 회사 웹사이트 등으로 즉시 연결됩니다. 앱 설치 없이 모든 스마트폰에서 호환됩니다."
    },
    {
  ...
  const reviews = [
    ...
    {
      text: "거래처 미팅 때 메탈 명함 건네니 분위기가 확 달라집니다. 뒷면 QR코드로 제 포트폴리오 바로 뜨는거 보고 다들 감탄하네요. 저를 확실하게 각인시킨 기분입니다.",
      author: "정*훈 대표",
      tier: "SIGNATURE",
      tag: "#비즈니스필수템"
    },
